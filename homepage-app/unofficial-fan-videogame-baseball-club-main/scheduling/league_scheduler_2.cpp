...
std::vector<ResidencyBlock> LeagueScheduler2::generateSeasonSchedule(std::vector<Team>& all_teams, int games_per_team) {
    ...
    int current_day = 1;
    const int MAX_SEASON_DAYS = 200; // A reasonable upper limit for the season
 ...
 *
 * v3.8.0: Logic is fine-tuned to ensure the number of games generated is always
 * consistent and that rest days are placed logically for player wellness and travel.
 */
ResidencyBlock LeagueScheduler2::createResidencyBlock(const Team& host, const std::vector<Team>& visitors, int start_day, int& out_duration_days) {
    ...
    // Flawless Game Generation: 2 games per visitor against the host (6 total)
    for (const auto& visitor : visitors) {
        Game g1, g2;
    ...
    // Flawless Game Generation: 2 games per visitor pair (6 total)
    ...
    // Total duration is the final day offset + 1 for departure
...
std::vector<Game> LeagueScheduler2::generateNeutralSiteSeries(const Team& visitor1, const Team& visitor2, const Team& host_stadium, int num_games, GameType game_type, int start_day, int& day_offset) {
    ...
    for (int i = 0; i < num_games; ++i) {
        ...
        day_offset += (i < num_games - 1) ? 2 : 1; // Add a rest day between games in the series
    }
    day_offset++; // Rest day after the series concludes
    return series_games;
}
