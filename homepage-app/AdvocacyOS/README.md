...
*   **Evidence Vault**: A secure, encrypted space to upload and tag supporting files:
    *   Screenshots of text messages or emails.
    *   Scans of official letters and documents.
    *   Photos of property damage.
    *   Audio recordings of phone calls (where legally permissible).
*   **Case File Management**: The ability to create separate, distinct "case files" (e.g., "Housing Dispute," "SNAP Benefits Case," "Family Matter") and tag each piece of evidence to the relevant case.
*   **Automated Timeline Generator**: The "killer feature" that automatically generates a clean, chronological, easy-to-read timeline of events from a specific case file, which can be exported as a PDF.
*   **Secure Sharing Portal**: A feature that allows the user to generate a secure, time-limited, read-only link to a specific case file's timeline. This can be shared with an attorney, a legal aid worker, a trusted supporter (like a brother), or submitted as evidence. This is the evolution of the "Trusted Viewer" concept.
*   **Advocacy Resource Hub**: A curated, in-app library of links to national and local resources, such as the Americans with Disabilities Act (ADA) network, state-specific legal aid societies, and tenant rights organizations.
...
To strategically manage complexity and ensure a rapid deployment of a valuable tool, AdvocacyOS adopts a phased approach for integrating its feedback system.
...
*   **Functionality:** After the AI provides a task breakdown, a simple rating widget appears, capturing **Accuracy, Safety, Tone, and Actionability**.
*   **Data Storage:** This feedback is securely saved to AdvocacyOS's own **Firestore database**.
*   **Benefits:** This approach allows AdvocacyOS to capture the essential function of the evaluation workflow without the overhead of a new third-party API, ensuring that invaluable safety and quality data can be gathered from day one.

###### Phase 2: Post-Launch (Evaluating a Formal API)
Once AdvocacyOS is in use, the project will be in a much stronger position to evaluate a formal API integration.
*   **Informed Decision:** With real-world usage, AdvocacyOS will have a deep understanding of the specific features required from an external API for membership integration.
